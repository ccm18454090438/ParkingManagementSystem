<template>
  <div>
    <el-form :model="orderRequestParams" size="small" :inline="true" label-width="68px">
      <el-form-item label="停车场" prop="parkinglotid">
        <el-select v-model="orderRequestParams.parkinglotName" placeholder="请选择停车场" clearable style="width: 240px">
      ...
      <el-form-item label="车牌号" prop="carplate">
        <el-input v-model="orderRequestParams.carplate" clearable style="width: 120px" />
      </el-form-item>
      <el-form-item label="入库日期">
        <el-date-picker v-model="orderRequestParams.dateRange" style="width: 240px" value-format="yyyy-MM-dd HH:mm:ss"
    ...
    <el-table id="exportTab" :data="tableData" stripe
      ...
      <el-table-column prop="type" label="收入类型" align="center">
        <template slot-scope="scope">
          <span v-show="scope.row.type == 0">临时停车</span>
          <span v-show="scope.row.type == 1">月卡充值</span>
          <span v-show="scope.row.type == 2">季卡充值</span>
          <span v-show="scope.row.type == 3">年卡充值</span>
        </template>
      </el-table-column>
      <el-table-column prop="method" label="支付方式" align="center">
        <template slot-scope="scope">
          <span v-show="scope.row.method == 0">现金</span>
          <span v-show="scope.row.method == 1">支付宝</span>
          <span v-show="scope.row.method == 2">微信</span>
          <span v-show="scope.row.method == 3">会员卡</span>
        </template>
      </el-table-column>
      <el-table-column prop="amount" label="收入金额/元" align="center"></el-table-column>
      ...
      <el-table-column prop="status" label="已支付" align="center">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    load() {
      this.orderRequestParams.pageParams = this.pageParams
      this.$request({
        url: '/order/getAll',
        method: 'POST',
        data: this.orderRequestParams
      })
        .then((res) => {
          console.log(res.data);
          this.tableData = res.data.items;
          this.tableData.duration = this.formatTime(res.data.items.duration)
    ...
    resetQuery() {
      this.orderRequestParams.dateRange = [];
