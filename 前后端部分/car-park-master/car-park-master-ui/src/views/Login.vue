<template>
  <div style="height: 100vh; display: flex; align-items: center; justify-content: center; background-color: #0f9876">
    <div style="display: flex; background-color: white; width: 30%; border-radius: 5px; overflow: hidden; padding: 20px">
      <!-- <div style="flex: 1">
        <img src="@/assets/login.png" alt="" style="width: 100%">
      </div> -->
      <div style="flex: 1; display: flex; align-items: center; justify-content: center">
        <el-form :model="user" style="width: 80%" :rules="rules" ref="loginRef">
          <div style="font-size: 20px; font-weight: bold; text-align: center; margin-bottom: 20px">欢迎登录智慧停车后台系统</div>
        ...
        if (valid) {
          ...
          this.$request.post('/auth/login', this.user).then(res => {
            ...
            if (res.code === 0) {
              ...
              localStorage.setItem("honey-user", JSON.stringify(res.data.user))  // 存储用户数据
              localStorage.setItem("token", JSON.stringify(res.data.token))  // 存储token
            } else {
              this.$message.error(res.msg)
            }
          })
